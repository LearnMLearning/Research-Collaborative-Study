...
**2. 函数近似能力**
前馈神经网络具有强大的函数近似能力。通用近似定理 (universal approximation theorem) 指出，存在一个二层前馈神经网络，具有一个线性输出层和一个隐层，其中隐层含有充分数量的神经元，激活函数为挤压函数，这个网络可以以仁义精度近似任意一个在紧的定义域上连续函数。从这个意义上，前馈神经网络的函数近似能力是通用的。
