...
(1) 用于回归。神经网络的输出层只有一个神经元，其输出是一个实数值。神经网络表示为 $y=f(\mathbf x)$，其中 $y\in \mathcal R$。预测时给定输入 $\mathbf x$，计算输出 $y$。
(2) 用于二类分类。神经网络的输出层只有一个神经元，其输出是一个概率值。神经网络表示为 $p=P(y=1|\mathbf x)=f(\mathbf x)$，其中 $y\in \{0,1\}$，满足条件
$$
